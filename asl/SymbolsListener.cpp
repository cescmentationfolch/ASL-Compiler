...
void SymbolsListener::exitFunction(AslParser::FunctionContext *ctx) {
  ...
  if (Symbols.findInCurrentScope(ident)) {
    ...
    for(unsigned int i = 0; i < ctx->parameters().size(); ++i){
        TypesMgr::TypeId t = getTypeDecor(ctx->parameters(i));
        lParamsTy.push_back(t);
    }
    TypesMgr::TypeId tRet;
    if(ctx->vartype())
        tRet = getTypeDecor(ctx->vartype());
    else
...
void SymbolsListener::exitVariable_decl(AslParser::Variable_declContext *ctx) {
  //loop a todas las variables declaradas en una linea
...
void SymbolsListener::enterVartype(AslParser::VartypeContext *ctx) {
  DEBUG_ENTER();
}
void SymbolsListener::exitVartype(AslParser::VartypeContext *ctx) {
    TypesMgr::TypeId t1;
    if(ctx->type())
      t1 = getTypeDecor(ctx->type());
    else
      t1 = getTypeDecor(ctx->array());
    putTypeDecor(ctx, t1);
  DEBUG_EXIT();
}
...
void SymbolsListener::exitArray(AslParser::ArrayContext *ctx) {
  TypesMgr::TypeId tprimitive = getTypeDecor(ctx->type());
  int size = std::stoi(ctx->INTVAL()->getText());
  TypesMgr::TypeId t = Types.createArrayTy(size, tprimitive);
  putTypeDecor(ctx, t);
  DEBUG_EXIT();
}
...
void SymbolsListener::enterLogicalOp(AslParser::LogicalOpContext *ctx) {
  DEBUG_ENTER();
}
void SymbolsListener::exitLogicalOp(AslParser::LogicalOpContext *ctx) {
  DEBUG_EXIT();
}

void SymbolsListener::enterUnary(AslParser::UnaryContext *ctx) {
  DEBUG_ENTER();
}
void SymbolsListener::exitUnary(AslParser::UnaryContext *ctx) {
  DEBUG_EXIT();
}
